'use strict';
// V6: photo-captions end-to-end /create -> /review -> /annotate
...
const SHAPE = 'photo-captions';

test(`V6: ${SHAPE} end-to-end /create -> /review -> /annotate`,
  { skip: process.env.CI === 'true' ? SKIP_REASON : false, timeout: 240000 }, async () => {
    const runId = `e2e-v6-${Date.now()}`;
    const fixturePath = path.resolve(__dirname, 'fixtures', 'v6-captions.md');
