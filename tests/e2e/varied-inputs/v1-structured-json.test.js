'use strict';
// V1: structured-json end-to-end /create -> /review -> /annotate
...
const SHAPE = 'structured-json';

test(`V1: ${SHAPE} end-to-end /create -> /review -> /annotate`,
  { skip: process.env.CI === 'true' ? SKIP_REASON : false, timeout: 240000 }, async () => {
    const runId = `e2e-v1-${Date.now()}`;
    const fixturePath = path.resolve(__dirname, 'fixtures', 'v1-brief.json');
    ...
    // 1. /instadecks:create with --brief (structured JSON)
    const create = spawnSync('node', [createCli, '--brief', fixturePath, '--run-id', runId],
      { encoding: 'utf8', timeout: 180000 });
    assert.equal(create.status, 0, `create failed: status=${create.status}\nstderr:\n${create.stderr}`);

    // 2. Assert deck artifacts
    ...
    const annotatedExists = fs.existsSync(path.join(runDir, 'annotated.pptx'))
      || fs.readdirSync(runDir).some(f => f.endsWith('annotated.pptx') || /annotated/i.test(f));
