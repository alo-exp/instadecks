'use strict';
// V4: one-line-ask end-to-end /create -> /review -> /annotate
...
const SHAPE = 'one-line-ask';

test(`V4: ${SHAPE} end-to-end /create -> /review -> /annotate`,
  { skip: process.env.CI === 'true' ? SKIP_REASON : false, timeout: 240000 }, async () => {
    const runId = `e2e-v4-${Date.now()}`;
    const fixturePath = path.resolve(__dirname, 'fixtures', 'v4-ask.txt');
    assert.ok(fs.existsSync(fixturePath), `missing fixture: ${fixturePath}`);
    const briefText = fs.readFileSync(fixturePath, 'utf8').trim();
