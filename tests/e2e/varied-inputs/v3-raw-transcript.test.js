'use strict';
// V3: raw-transcript end-to-end /create -> /review -> /annotate
...
const SHAPE = 'raw-transcript';

test(`V3: ${SHAPE} end-to-end /create -> /review -> /annotate`,
  { skip: process.env.CI === 'true' ? SKIP_REASON : false, timeout: 240000 }, async () => {
    const runId = `e2e-v3-${Date.now()}`;
    const fixturePath = path.resolve(__dirname, 'fixtures', 'v3-transcript.txt');
    assert.ok(fs.existsSync(fixturePath), `missing fixture: ${fixturePath}`);
    const briefText = fs.readFileSync(fixturePath, 'utf8');
    ...
    const create = spawnSync('node', [createCli, '--brief-text', briefText, '--run-id', runId],
