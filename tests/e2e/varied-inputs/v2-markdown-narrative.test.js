'use strict';
// V2: markdown-narrative end-to-end /create -> /review -> /annotate
...
const SHAPE = 'markdown-narrative';

test(`V2: ${SHAPE} end-to-end /create -> /review -> /annotate`,
  { skip: process.env.CI === 'true' ? SKIP_REASON : false, timeout: 240000 }, async () => {
    const runId = `e2e-v2-${Date.now()}`;
    const fixturePath = path.resolve(__dirname, 'fixtures', 'v2-brief.md');
